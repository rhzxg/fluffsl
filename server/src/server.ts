...
const legend: SemanticTokensLegend = { tokenTypes: ['keyword', 'type', 'function', 'variable', 'macro', 'enumMember', 'comment', 'number', 'string'], tokenModifiers: ['declaration', 'definition', 'readonly'] };
...
// FSL uses these identifiers in DATA/RES declarations and as shader IO
// semantics. They are language symbols, not user-defined variables.
const builtinSlots = new Set([
  'POSITION', 'POSITION0', 'POSITION1', 'POSITIONX', 'POSITIONY', 'POSITIONZ',
  'NORMAL', 'NORMAL0', 'NORMAL1', 'TANGENT', 'BITANGENT', 'COLOR', 'COLOR0',
  'COLOR1', 'BLENDINDICES', 'BLENDWEIGHT', 'TEXCOORD', 'TEXCOORD0',
  'TEXCOORD1', 'TEXCOORD2', 'TEXCOORD3', 'TEXCOORD4', 'TEXCOORD5',
  'TEXCOORD6', 'TEXCOORD7', 'TEXCOORD8', 'TEXCOORD9', 'SV_Position',
  'SV_POSITION', 'SV_Coverage', 'SV_COVERAGE', 'SV_Target', 'SV_TARGET',
  'SV_Depth', 'SV_DEPTH', 'SV_VertexID', 'SV_VERTEXID', 'SV_InstanceID',
  'SV_INSTANCEID', 'SV_PrimitiveID', 'SV_PRIMITIVEID', 'SV_IsFrontFace',
  'SV_ISFRONTFACE', 'SV_RenderTargetArrayIndex', 'SV_RENDERTARGETARRAYINDEX',
  'SV_GroupID', 'SV_GROUPID', 'SV_GroupIndex', 'SV_GROUPINDEX',
  'SV_GroupThreadID', 'SV_GROUPTHREADID', 'SV_DispatchThreadID',
  'SV_DISPATCHTHREADID', 'SV_SampleIndex', 'SV_SAMPLEINDEX', 'SV_PointSize',
  'SV_POINTSIZE', 'SV_DomainLocation', 'SV_DOMAINLOCATION',
  'SV_OutputControlPointID', 'SV_OUTPUTCONTROLPOINTID', 'SV_ShadingRate',
  'SV_SHADINGRATE', 'SV_TessFactor', 'SV_TESSFACTOR', 'SV_InsideTessFactor',
  'SV_INSIDETESSFACTOR'
]);
const builtinUpdateFrequency = new Set(['UPDATE_FREQ_NONE', 'UPDATE_FREQ_PER_FRAME', 'UPDATE_FREQ_PER_BATCH', 'UPDATE_FREQ_PER_DRAW', 'UPDATE_FREQ_USER']);
const builtinTypes = new Set(fallbackTypes); const builtinFns = new Set(builtinFunctions); const builtinMacros = new Set<string>();

function isBuiltinSlot(name: string): boolean {
  return builtinSlots.has(name) || /^(?:POSITION|NORMAL|TANGENT|BITANGENT|COLOR|TEXCOORD|BLENDINDICES|BLENDWEIGHT)[A-Za-z0-9]*$/.test(name) || /^SV_[A-Za-z][A-Za-z0-9_]*$/.test(name);
}
  ...
  for (const root of candidates) {
    if (root && fs.existsSync(root)) {
      ...
      for (const file of fs.readdirSync(inc).filter(x => x.endsWith('.h'))) {
        ...
        for (const m of text.matchAll(/^(?![ \t]*#)[ \t]*(?:inline[ \t]+)?(?:[A-Za-z_]\w*(?:[ \t]*<[^;{}()]+>)?)[ \t]+([A-Za-z_]\w*)[ \t]*\([^;{}]*\)/gm)) { builtinFns.add(m[1]); }
...
function parse(uri: string, text: string): Index {
  ...
  // Horizontal whitespace is intentional: using \s here joins a preprocessor
  // identifier to DATA/RES on the next line and creates a fake function scope.
  const fn = /^(?![ \t]*#)[ \t]*([A-Za-z_]\w*(?:[ \t]*[0-9]+x[0-9]+)?)[ \t]+([A-Za-z_]\w*)[ \t]*\(/gm; let fnMatch: RegExpExecArray | null;
  while ((fnMatch = fn.exec(lexical.code))) { const returnType = fnMatch[1], name = fnMatch[2]; if (/^(return|if|else|for|while|switch|case|defined)$/.test(returnType) || /^(if|for|while|switch|in|out|inout|DATA|RES|STRUCT|CBUFFER|PUSH_CONSTANT)$/.test(name)) { continue; } const openParen = lexical.code.indexOf('(', fnMatch.index + fnMatch[0].length - 1), closeParen = matchingParen(lexical.code, openParen); if (closeParen < 0) { continue; } add(name, 'function', fnMatch.index + fnMatch[0].lastIndexOf(name), returnType.trim()); parameterSpans.push({ start: openParen, end: closeParen }); const argsText = lexical.code.slice(openParen + 1, closeParen), parameters = new Set<string>(); let cursor = 0; for (const arg of splitArgs(argsText)) { const argOffset = argsText.indexOf(arg, cursor); cursor = argOffset + arg.length; const pm = /([A-Za-z_]\w*)\s*(?:\[[^]]*\])?$/.exec(arg); if (pm) { parameters.add(pm[1]); add(pm[1], 'parameter', openParen + 1 + argOffset + arg.lastIndexOf(pm[1])); } } let bodyOpen = closeParen + 1; while (bodyOpen < lexical.code.length && /\s/.test(lexical.code[bodyOpen])) { bodyOpen++; } scopes.push({ start: openParen, end: lexical.code[bodyOpen] === '{' ? matchingBrace(lexical.code, bodyOpen) : closeParen, parameters }); }
  for (const lineMatch of lexical.code.matchAll(/.*(?:\r\n|\n|\r|$)/g)) {
    ...
    for (const m of code.matchAll(/\b[A-Za-z_]\w*\b/g)) { const n = m[0], off = base + (m.index ?? 0); if (keywords.has(n)) { addToken(n, off, 0); } else if (builtinTypes.has(n)) { addToken(n, off, 1); } else if (builtinFns.has(n) && /\s*\(/.test(code.slice((m.index ?? 0) + n.length))) { addToken(n, off, 2); } else if (isBuiltinSlot(n)) { addToken(n, off, 5); } else if (builtinUpdateFrequency.has(n) || builtinMacros.has(n)) { addToken(n, off, 4); } }
    ...
    const decl = /\b([A-Za-z_]\w*(?:[0-9]+x[0-9]+)?)\s+([A-Za-z_]\w*)\s*(?==|;|\[)/g; let m: RegExpExecArray | null; while ((m = decl.exec(code))) { const absolute = base + m.index; if (parameterSpans.some(span => absolute >= span.start && absolute < span.end)) { continue; } if (!['if', 'for', 'while', 'return'].includes(m[1]) && !symbols.some(s => s.name === m![2] && s.range.start.line === doc.positionAt(base).line)) { add(m[2], 'variable', absolute + m[0].lastIndexOf(m[2]), m[1]); } }
  }
  for (const s of symbols) { addToken(s.name, doc.offsetAt(s.range.start), s.kind === 'type' ? 1 : s.kind === 'function' ? 2 : s.kind === 'macro' ? 4 : 3, 1); }
...
connection.onHover((p: TextDocumentPositionParams): Hover | null => { const n = tokenAt(p.textDocument.uri, p.position); if (!n) { return null; } const s = locations(n)[0]; if (!s && !builtinTypes.has(n) && !builtinFns.has(n) && !builtinMacros.has(n) && !builtinUpdateFrequency.has(n) && !isBuiltinSlot(n)) { return null; } const k = s?.kind || (builtinTypes.has(n) ? 'built-in type' : builtinFns.has(n) ? 'built-in function' : builtinUpdateFrequency.has(n) ? 'FSL update frequency' : isBuiltinSlot(n) ? 'FSL shader slot' : 'built-in macro'); return { contents: { kind: MarkupKind.Markdown, value: `**${k}** \`${n}\`${s?.detail ? '\n\n`' + s.detail + '`' : ''}` } }; });
